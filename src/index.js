...
function onCountryInput() {
  ...
  fetchCountries(name)
    .then(country => {
      ...
      if (country.length === 1) {
        countryInfo.insertAdjacentHTML('beforeend', markCountryInfo(country));
      } else if (country.length >= 10) {
        ifTooManyMatchesAlert();
      } else {
        countryList.insertAdjacentHTML('beforeend', markCountryList(country));
      }
    })
    .catch(ifWrongName);
}
...
function ifWrongName() {
  Notiflix.Notify.failure('Oops, there is no country with that name');
}

function markCountryList(country) {
...
function markCountryInfo(country) {
